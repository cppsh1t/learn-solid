...
function processRouterConfigItem(parentUrl: string, config: RouterConfigItem) {
  ...
  const processedConfig: ProcessedConfig = config.independent ? { url: config.url, component: config.component } : { url: `${parentUrl}${config.url}`, component: config.component }
  arr.push(processedConfig)
  if (config.children) {
    for (const childConfig of config.children) {
      const childArr = processRouterConfigItem(`${parentUrl}${config.url}`, childConfig)
      arr.push(...childArr)
    }
  }
  return arr
